...
const protect = asyncHandler(async (req, res, next) => {
    ...
    let refreshToken = req.cookies?.refreshToken; // Assuming the refresh token is stored in cookies
    ...
    try {
        ...
        if (refreshToken) {
            try {
                ...
                const decodedRefresh = jwt.verify(refreshToken, process.env.REFRESH_TOKEN_SECRET || "default_refresh_key");
                
                // Find the admin using the decoded refresh token
                const admin = await Admin.findById(decodedRefresh.id).select("-password");
                if (!admin) {
                    throw new Error("Refresh token is invalid or user not found");
                }
